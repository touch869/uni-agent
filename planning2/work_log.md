...
|---|---|---|---|---|---|---|
| **0.7** | 145:0.42(G42%·M28%) | 147:0.80(G92%·M25%) | 146:128✓(log失) | 未完成 | 147:0.19(G36%·M32%)‡ | 未完成 |
| **0.8** | 146:0.26(G41%·M19%)‡ | 145:0.86(G64%·M24%) | 147:0.20(G43%·M31%)‡ | 未完成 | 未完成 | 未完成 |

> 格式: `host:吞吐比(G=GPU prefix%, M=mooncake External%)`。‡=含stall/restart。**7/12 backup完成**。α0.4最优(0.80-0.86)。control/mc均无驱逐(preempt=0)。128已收尾→256进行中。

## 256 样本搜索结果矩阵（α × load_threshold）
**值 = 256对照组耗时 / 该组耗时（同机），>1.0 = mc 比对照快**。256对照进行中(~35-45min/台)，baseline 待出。threshold=0.7 focus(最佳)。host=147/145/146(144补128)。

| load_th\α | 0.4 | 0.5 | 0.6 |
|---|---|---|---|
| **0.7** | 待跑 | 待跑 | 待跑 |

> 对照完成后填。256 对照 baseline 与 mc 256 对比，看 α 趋势是否同 128 一致。
