"""Config for the mock sandbox deployment.

``MockDeploymentConfig`` is the ``type: mock`` member of the ``DeployConfig``
discriminated union -- selectable from ``agent_config`` exactly like
host/local/modal. It carries the tunables that control MockRuntime's
...
class MockDeploymentConfig(BaseModel):
    """Configuration for the mock (CPU-only, canned-observation) deployment.
    ...
    templates (see ``mock/deployment.py``).
    """
    ...
    observation_scale: float = Field(default=1.0, gt=0.0)
    """Multiplier applied to every template's length. 1.0 = as-authored;
    use >1 to stress KV/prefill load. Must be >0; a value <=1 shrinks output."""
    ...
    # deployment block, so MockDeploymentConfig receives docker-only keys it
    # doesn't model. Ignoring them lets the same dataset drive both local and
    # mock deployments without a per-sample schema fork.
    model_config = ConfigDict(extra="ignore")

    def get_deployment(self, run_id: str):
        from .deployment import MockDeployment

        return MockDeployment.from_config(self, run_id)
