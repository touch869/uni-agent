...
| `RouteDataProvider` | **uni-agent** `llm_router/types.py` | `get_metric(id, key) → float\|int`, `get_metrics(id) → ReplicaMetrics`, `get_gpu_prefix_hit_rate(id, prompt_ids) → float`, `get_tier_prefix_hit_rate(id, prompt_ids, tier) → float` | — |
...
> **说明**：策略权重（`weight`）由 `StrategyConfig.weight` 持有，`KVCacheAwareStrategy` 本身不存储，由 Balancer 在调用 `route()` 时作为 tuple 第二项传入。
...
$$S_{load} = \frac{1 - kv\_cache\_usage\_perc}{1 + num\_requests\_running + num\_requests\_waiting}$$

| 字段 | 含义 | 数据来源 |
|------|------|----------|
| `kv_cache_usage_perc` | GPU KV cache 使用率（0~1） | `RouteDataProvider.get_metrics(replica_id)` → `ReplicaMetrics.get(MetricKey.KV_CACHE_USAGE_PERC)` |
| `num_requests_running` | 当前在模型中执行的请求数 | `ReplicaMetrics.get(MetricKey.NUM_REQUESTS_RUNNING)` |
| `num_requests_waiting` | 等待队列中的请求数 | `ReplicaMetrics.get(MetricKey.NUM_REQUESTS_WAITING)` |

`S_load ∈ [0, 1]`：`kv=0, r=0, w=0` → `S_load=1`（最空闲）；`kv=1` 或请求队列极深 → `S_load→0`（高负载）。

### 6.2 过载判定

$$replica\ i\ \text{过载} \iff S_{load,i} < load\_threshold$$

`load_threshold ∈ (0, 1)` 是可配置的过载分界线（默认 0.1）。`S_load` 得分低于阈值即认为该 replica 负载过重，不宜接收新请求。

> 注意：`S_load = load_threshold`（恰好等于阈值）**不算过载**，仍参与正常打分。
...
**慢路径**：多级缓存命中率加权（HBM 不参与）。命中率基于 **GPU 驱逐表** per-request 查询（见 §6.6）。

$$S_{cache,i} = \sum_{layer} hit\_rate_{layer,i} \times w_{layer}$$

cpu 层和 ssd 层对应同一组 block 在不同存储层中的互斥状态（一个 block 只存在于一层），因此加权和天然 ≤ 1，无需截断。
...
$$score_i = \begin{cases}
\alpha \times S_{cache,i} + (1-\alpha) \times S_{load,i} & \text{全部过载（慢路径 cache + load 共同区分）} \\[4pt]
(1-\alpha) \times S_{load,i} & \text{部分过载中的过载 replica（cache 置零，无法因高命中翻身）} \\[4pt]
\alpha \times S_{cache,i} + (1-\alpha) \times S_{load,i} & \text{健康 replica（正常加权）}
\end{cases}$$

设计要点：

- **部分过载中的过载 replica**：cache 分强制置零，只保留 `(1-α)·S_load`。得分上界为 `(1-α)·load_threshold`，而健康 replica 得分下界也为 `(1-α)·load_threshold`（S_load 恰好等于 threshold 时）。因此过载 replica 严格排在所有健康 replica 之后，无论 cache 命中率多高——防止高 cache 的过载 replica 持续接收流量加剧其负载压力。
- **全部过载**：池内无健康 replica 可选时，cache 和 load 共同参与打分。tier cache 信号（慢路径）与 S_load 大小共同区分优劣，负载较轻、命中更好的 replica 排前面。
- **健康 replica**：`S_load ∈ [load_threshold, 1]`，`S_cache ∈ [0, 1]`，α 加权后得分 `∈ [0, 1]`。
...
| vLLM 负载指标不可用 | `kv_cache_usage_perc / num_requests_*` 取默认值 0 / 0 → `S_load = 1`（乐观，避免误判过载） |
...
| `load_threshold` | `float` | `0.1` | `(0, 1)` 开区间 | 过载分界线；`S_load < threshold` 时视为过载。0.1 对应「kv=90% 且无队列」或「中等 kv + 少量并发」时触发过载 |
| `layer_weights` | `dict[str, float] \| None` | `None`（→ `{"cpu": 1.0, "ssd": 0.25}`） | `None` 用默认；显式 dict 的键须为 `{"cpu", "ssd"}` 的子集，各值 `≥ 0` | 慢路径各级缓存权重；v1 仅 `cpu` 层生效（查 GPU 驱逐表），`ssd` 层留位待接口（见 §6.6） |

> **注**：组合框架层的策略权重（`weight`）由 `StrategyConfig.weight` 持有，不在 `KVCacheAwareStrategy` 构造参数中，由 Balancer 在调用 `route()` 时传入。
...
@StrategyRegistry.register("kv_cache_aware")
class KVCacheAwareStrategy:
    def __init__(self, alpha=0.7, load_threshold=0.1, layer_weights=None):
        if not 0 <= alpha <= 1:
        ...
        for tier, w in layer_weights.items():
            if tier not in {"cpu", "ssd"}:
                raise StrategyError(f"layer_weights key must be in {{'cpu','ssd'}}, got '{tier}'")
            if w < 0:
    ...
    def score(self, ctx, metrics, replicas) -> list[float]:
        ...
        # 1. S_load per replica; overloaded when S_load < threshold
        load_scores, is_overloaded = [], []
        for replica in replicas:
            m = metrics.get_metrics(replica.replica_id)
            ...
            is_overloaded.append(s_load < self.load_threshold)
        all_overloaded = all(is_overloaded)

        # 2. All overloaded → slow path, skip GPU hit queries
        if all_overloaded:
            return [
                self.alpha * self._slow_cache(metrics, replica, prompt_ids)
                + (1 - self.alpha) * load_scores[idx]
                for idx, replica in enumerate(replicas)
            ]

        # 3. Partial overload: query GPU hit for healthy replicas only
        gpu_hits = [
            0.0 if is_overloaded[idx]
            else metrics.get_gpu_prefix_hit_rate(replica.replica_id, prompt_ids)
            for idx, replica in enumerate(replicas)
        ]
        ...
        for idx, replica in enumerate(replicas):
            if is_overloaded[idx]:
                ...
                s_cache = gpu_hits[idx] if use_fast else self._slow_cache(metrics, replica, prompt_ids)
                result.append(self.alpha * s_cache + (1 - self.alpha) * load_scores[idx])
        return result

    def _slow_cache(self, metrics, replica, prompt_ids) -> float:
        # cpu/ssd tiers are mutually exclusive; weighted sum is naturally ≤ 1
        return sum(
            metrics.get_tier_prefix_hit_rate(replica.replica_id, prompt_ids, tier) * w
...
```mermaid
flowchart TD
    A["score(ctx, metrics, replicas)"] --> B["逐 replica: S_load = (1-kv)/(1+r+w), overloaded = S_load < threshold"]
    B --> C{"全部过载?"}
    C -->|是| D["use_fast = False (强制慢路径，跳过 GPU hit 查询)"]
    ...
    F --> G
    G --> H{"全部过载?"}
    H -->|是| I["score = α × S_cache + (1-α) × S_load"]
    H -->|否| J{"该 replica 过载?"}
    J -->|是| K["score = (1-α) × S_load  (cache 置零，严格排在健康 replica 之后)"]
    J -->|否| L["score = α × S_cache + (1-α) × S_load"]
    I --> M["返回 scores 列表"]
    K --> M
    L --> M
```
...
| 场景 | 处理 |
|------|------|
| 构造参数非法（`alpha ∉ [0,1]`、`load_threshold ≤ 0`、`layer_weights` 含负值或非法 key） | 构造时抛 `StrategyError`（快速失败）。Config 层（`KVCAwareStrategyConfig`）要求键恰好为 `{"cpu","ssd"}`，抛 `ConfigError`；Strategy 层要求键为该集合子集，抛 `StrategyError` |
...
| weight 归属 | KVCacheAwareStrategy 持有 / StrategyConfig 持有 | **StrategyConfig 持有** | weight 是配置关注点；runtime strategy 只负责打分，不持有框架层权重 |
| 负载公式 | `kv_usage × (running + waiting)` / `(1-kv)/(1+r+w)` | **`(1-kv)/(1+r+w)`** | 直接得到 [0,1] 的空闲度分数，无需额外归一化；与业界方案（openeuler/gateway）一致；threshold 可直接作为分数分界线 |
| 过载判定 | `S_load < 0`（旧公式产生负数）/ `S_load < threshold`（新公式 [0,1]） | **`S_load < threshold`** | 新公式 S_load 天然在 [0,1]，用 threshold 作为显式分界线，语义清晰 |
| 过载 replica 打分 | 直接取 `S_load`（负数沉底）/ cache 置零 `(1-α)·S_load` | **cache 置零 `(1-α)·S_load`** | S_load 不再是负数，无法用负数沉底；cache 置零后得分上界 = `(1-α)·threshold`，严格低于任意健康 replica（得分下界同为 `(1-α)·threshold`） |
| 慢路径加权和截断 | 截断 [0,1] / 不截断 | **不截断** | cpu/ssd tier 互斥（block 只存在于一层），加权和天然 ≤ 1，无需额外截断 |
