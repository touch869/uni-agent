...
router** — hosted in the `uni_agent.llm_router` package and injected into verl
by FQN (`rollout.router_config_path` + a `pkg://` router YAML; no verl-side
registration needed). Sessions run on the uni-agent **framework + task_runner
path** (`uni_agent.framework.task_runner.run_task`): vLLM replicas sit behind
the kvcaware router (KV-cache hit rate + load aware dispatch), a gateway pool
drives `react` agent sessions in openyuanrong remote sandboxes, and each task's
reward is reported back. No trainer is started.
...
creds + observability env) and forwards all CLI flags to
`parallel_infer_verl_kvc.py`. `--task-config` is required — it selects the
...
metrics. At the end, `parallel_infer_verl_kvc.py` prints an `inference summary`
...
- The task runner requires an AKernel remote sandbox (openyuanrong); without it
  sessions fail fast. The old blackbox-recipes / swe-rex agent configs were
  removed in the framework migration.
...
- Keep the task config's `max_total_tokens` comfortably below `--max-model-len`;
  the react agent budgets per-session tokens against it and vLLM rejects
  requests that exceed the engine context.
