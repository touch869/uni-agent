...
Migrated example: the uni-agent framework path (AgentFrameworkRolloutAdapter + gateway
sessions + ``--task-config`` + TransferQueue + ``_read_rm_scores``) plus the KV-cache-aware
router, wired through verl's *plugin* mechanism (``rollout.router_config_path``). This is
the 144-verified ``examples/inference/parallel_infer_verl_kvc.py`` adapted to verl's
current FQN-injected router plugin: the old ``rollout/router@...=kvcaware`` Hydra group no
longer exists in the vendored verl, so strategy overrides are applied to the packaged
``uni_agent/llm_router/configs/kvc_aware_router.yaml`` and the temp copy is handed to verl
via ``rollout.router_config_path``.
...
``--task-config`` is required (same YAML shape as ``examples/inference/parallel_infer_verl.py``);
the policy endpoint is the gateway session, bound by the runner, not a flag.
...
# Default router plugin YAML (FQN-injected; "router本身没问题"). Strategy overrides are
# applied to this file and the temp copy is passed to verl via rollout.router_config_path.
...
def _write_overridden_router_yaml(
    *,
    ...
    # Load with Hydra defaults expansion (NOT plain OmegaConf.load), matching verl's
    # ``_load_router_yaml`` semantics exactly (the packaged YAML's defaults-referenced
    # sub-configs are merged before overrides are applied).
    ...
    # Composed strategies is a dict keyed by strategy name (defaults composition) or a
    # list (single-file plugin YAML). Duck-type: .keys() is the reliable probe for both.
    ...
    # Save the COMPOSED tree (defaults already expanded) so verl's loader can read the
    # temp file with plain Hydra compose (a defaults-free file composes to itself) or
    # OmegaConf.load alike.
...
def init_config(args: argparse.Namespace, *, task_configs: list[dict], served_model_name: str):
    ...
    This is the 144-verified ``parallel_infer_verl_kvc.py``'s ``init_config`` with the
    ...
    # Load real weights: the vendored verl's committed rollout.yaml still defaults to
    # ``load_format: dummy`` (random weights => the policy emits garbage). Force ``auto``
    # so the engine loads the actual checkpoint (matches the 144-verified run).
    ...
    # Optional KVCAware router strategy[0] overrides -- each falls back to the packaged
    # kvc_aware_router.yaml value when the flag is omitted. The override lands on a temp
    # YAML copy (packaged defaults + CLI deltas) that verl loads via router_config_path.
