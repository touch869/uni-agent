"""Integration: drive a real AgentEnv end-to-end with a mock deployment.

This is the proof that ``type: mock`` plugs into the production path with
...
from uni_agent.deployment import MockDeploymentConfig  # noqa: E402
from uni_agent.interaction.env import AgentEnv, AgentEnvConfig  # noqa: E402


@pytest.mark.asyncio
async def test_agent_env_lifecycle_with_mock_deployment() -> None:
    """Full AgentEnv lifecycle on a mock sandbox installs tools and returns a
    representative observation for a real tool command."""
    env_config = AgentEnvConfig(
        deployment=MockDeploymentConfig(seed=1, observation_scale=1.0),
    ...
    try:
        ...
        # must all succeed (exit 0) -- the mock routes them to install.
...
@pytest.mark.asyncio
async def test_agent_env_finish_action_via_mock() -> None:
    """The submit tool's fixed ``echo '<<<Finished>>>'`` command surfaces the
    terminal signal through the real run_action wrapper."""
    env = AgentEnv(
        run_id="it-2",
        env_config=AgentEnvConfig(deployment=MockDeploymentConfig()),
    )
    await env.start()
    try:
