...
weights / add long-tail entries without touching code. ``MockRuntime`` loads
...
from uni_agent.deployment.mock.deployment import MockRuntime, load_templates  # noqa: E402
...
@pytest.mark.asyncio
async def test_templates_path_overrides_pool(tmp_path) -> None:
...
async def test_overridden_pool_seed_reproducible(tmp_path) -> None:
    ...
    rt1 = MockRuntime(run_id="t", seed=9, templates_path=str(yaml_path))
    rt2 = MockRuntime(run_id="t", seed=9, templates_path=str(yaml_path))
