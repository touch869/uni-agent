"""Render + determinism tests for MockRuntime.run_in_session.
...
from swerex.runtime.abstract import BashInterruptAction  # noqa: E402

from uni_agent.deployment.mock.deployment import MockRuntime  # noqa: E402

from .conftest import _bash  # noqa: E402
...
async def test_same_seed_reproduces_identical_sequence() -> None:
    ...
    rt1 = MockRuntime(run_id="t1", seed=42)
    rt2 = MockRuntime(run_id="t2", seed=42)
...
@pytest.mark.asyncio
async def test_different_seeds_produce_different_rng_streams() -> None:
    """Different global seeds must produce different derived per-key seeds,
    which means the two runtimes sample from independent RNG streams."""
    rt_a = MockRuntime(run_id="a", seed=1)
    rt_b = MockRuntime(run_id="b", seed=2)
    for key in ("editor:view", "python_script", "test_output", "search", "default"):
        assert rt_a._derive_seed(key) != rt_b._derive_seed(key), key


@pytest.mark.asyncio
async def test_observation_scale_stretches_length() -> None:
    """observation_scale multiplies the template length (>=1x grows it)."""
    rt_small = MockRuntime(run_id="t", seed=7, observation_scale=1.0)
    rt_big = MockRuntime(run_id="t", seed=7, observation_scale=2.0)
