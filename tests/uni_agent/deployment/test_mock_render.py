...
async def test_different_seed_likely_differs() -> None:
    ...
    pulls = 30
    a = [await MockRuntime(run_id="a", seed=1).run_in_session(_bash(cmd)) for _ in range(pulls)]
    b = [await MockRuntime(run_id="b", seed=2).run_in_session(_bash(cmd)) for _ in range(pulls)]
