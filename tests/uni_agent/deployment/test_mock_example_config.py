"""Validate the shipped agent_config_mock.yaml example.
...
asserts its deployment block parses into a MockDeploymentConfig with sane
...
def _example_config_path() -> Path:
    """Locate scripts/agent_config_mock.yaml relative to the repo root.
    ...
    for root in candidates:
        p = root / "scripts" / "agent_config_mock.yaml"
        if p.is_file():
            return p
    raise FileNotFoundError("agent_config_mock.yaml not found under repo root")


def test_example_config_parses_as_mock() -> None:
    ...
    assert isinstance(env_cfg.deployment, MockDeploymentConfig)
    assert env_cfg.deployment.type == "mock"
