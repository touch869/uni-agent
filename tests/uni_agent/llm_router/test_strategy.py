...
``uni_agent/llm_router/docs/design/detailed_strategy_module.md`` §8. Numeric baselines reuse
the spec's partial/full-overload scenario tables (threshold=80, alpha=0.7,
1-alpha=0.3, layer_weights={cpu:1.0, ssd:0.25}).

A ``FakeRouteDataProvider`` stands in for the future real provider; loads are
made readable by fixing kv_cache_usage_perc=1.0 so load == num_requests_running.
"""
...
from uni_agent.llm_router.types import MetricKey, ReplicaInfo, ReplicaMetrics, RouteContext
...
class FakeRouteDataProvider:
    """In-memory metrics. data[replica_id] = {running, gpu_hit, tiers}.

    Sets kv_cache_usage_perc=1.0 so load = 1.0 * num_requests_running = running.
    """
    ...
    def get_metric(self, replica_id: str, key: str) -> float | int:
        d = self._data.get(replica_id, {})
        if key == MetricKey.KV_CACHE_USAGE_PERC:
            return d.get("kv_cache_usage_perc", 1.0)
        if key == MetricKey.NUM_REQUESTS_RUNNING:
            return d.get("running", 0)
        if key == MetricKey.NUM_REQUESTS_WAITING:
            return d.get("waiting", 0)
        return d.get(key, 0.0)

    def get_metrics(self, replica_id: str) -> ReplicaMetrics:
        d = self._data.get(replica_id, {})
        m = ReplicaMetrics(replica_id=replica_id)
        m.set(MetricKey.KV_CACHE_USAGE_PERC, d.get("kv_cache_usage_perc", 1.0))
        m.set(MetricKey.NUM_REQUESTS_RUNNING, d.get("running", 0))
        m.set(MetricKey.NUM_REQUESTS_WAITING, d.get("waiting", 0))
        return m
...
def _load(running: int, **extra) -> dict:
    """Build a replica metrics dict with kv_cache_usage_perc=1.0 so load == running."""
    return {"running": running, **extra}
...
class TestKVCacheAwareLoad:
    ...
    def test_threshold_is_not_overload(self):
        ...
        m = FakeRouteDataProvider({"crit": _load(80, gpu_hit=0.5), "over": _load(120, gpu_hit=0.9)})
    ...
    def test_missing_metrics_optimistic(self):
        ...
        strat = KVCacheAwareStrategy()
        m = FakeRouteDataProvider({})  # nothing configured
    ...
    def test_empty_layer_weights_honored(self):
        ...
        m = FakeRouteDataProvider({"a": _load(0, gpu_hit=0.0, tiers={"cpu": 1.0, "ssd": 1.0})})
...
class TestKVCacheAwareScore:
    ...
    def test_ssd_zero_hit_contributes_nothing(self):
        strat = KVCacheAwareStrategy()
        m = FakeRouteDataProvider({"a": _load(0, gpu_hit=0.0, tiers={"cpu": 0.5, "ssd": 0.0})})
        scores = strat.score(CTX, m, _replicas("a"))
...
class TestStrategyContract:
    ...
    def test_score_signature_callable(self):
        ...
        strat = KVCacheAwareStrategy()
        assert strat.score(CTX, FakeRouteDataProvider({}), _replicas("a")) is not None

    def test_output_length_and_order(self):
        strat = KVCacheAwareStrategy()
        m = FakeRouteDataProvider({"a": _load(20, gpu_hit=0.9), "b": _load(40, gpu_hit=0.1)})
    ...
    def test_stateless_repeatable(self):
        strat = KVCacheAwareStrategy()
        m = FakeRouteDataProvider({"a": _load(40, gpu_hit=0.8), "b": _load(20, gpu_hit=0.5)})
        replicas = _replicas("a", "b")
...
class TestRoute:
    ...
    def test_empty_pool_raises(self):
        with pytest.raises(RuntimeError):
            route([(ConstantStrategy([]), 1.0)], CTX, FakeRouteDataProvider({}), [])

    def test_length_mismatch_raises(self):
        with pytest.raises(StrategyError):
            route([(BadLengthStrategy(), 1.0)], CTX, FakeRouteDataProvider({}), _replicas("a", "b"))
    ...
    def test_strategy_exception_wrapped_with_name(self):
        ...
        with pytest.raises(StrategyError) as exc_info:
            route([(RaisingStrategy(), 1.0)], CTX, FakeRouteDataProvider({}), _replicas("a"))
...
class TestWarnings:
    def test_all_overloaded_no_cache_warns(self, caplog):
        ...
        strat = KVCacheAwareStrategy()
        m = FakeRouteDataProvider({"a": _load(120), "b": _load(160)})  # all overloaded, no cache
    ...
    def test_all_overloaded_with_cache_no_warn(self, caplog):
        ...
        strat = KVCacheAwareStrategy()
        m = FakeRouteDataProvider({"a": _load(120, tiers={"cpu": 0.5}), "b": _load(160)})
        with caplog.at_level(logging.WARNING, logger="uni_agent.llm_router.strategy"):
            strat.score(CTX, m, _replicas("a", "b"))
...
class TestFromConfig:
    ...
    def test_from_config_scores_match_direct(self):
        ...
        m = FakeRouteDataProvider({"a": _load(40, gpu_hit=0.8), "b": _load(120)})
        replicas = _replicas("a", "b")
