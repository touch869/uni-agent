...
class TestLoadNormalized:
    ...
    def test_waiting_term(self):
        """kv=0, running=0, waiting=10 → wait_usage=10/11; load = 0.3·(10/11)."""
        assert load_normalized(0.0, 0, 10, max_num_seqs=64) == pytest.approx(0.3 * (10 / 11))
    ...
    def test_max_num_seqs_zero_safe(self):
        """max_num_seqs=0 → running_usage degrades to 1.0 (no division by zero)."""
        load = load_normalized(0.0, 5, 0, max_num_seqs=0)
        assert load == pytest.approx(0.3)  # 0.3·1.0
